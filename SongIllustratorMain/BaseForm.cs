...
namespace SongIllustrator {
	public partial class BaseForm : FormView {
		public BaseForm() {
			InitializeComponent();
		}
		MenuItemView editCurrentFrameToolStripMenuItem;
		MenuItemView stopEditingCurrentFrameToolStripMenuItem;
		...
		public BaseForm(Factory globalSongIllustratorWin32Factory) {
		...
		private void CheckImage() {
			...
			foreach (LightPad launchPad in panel1.FormControls) {
				...
				for (int z = 0; z < launchPads[i].FrameData.Count; z++) {
					...
					if (currentPos * 100 >= time.TimeStamp) {
						if (!time.Fired) {
							...
							if (panel1.FormControls.Count > 0) {
								(panel1.FormControls[i] as LightPad).SetFrame(z);
							}
							//if (display.Canvas.FormControls.Count > 0) {
							//  LightPad pad = (display.Canvas.FormControls[i] as LightPad);
							//  if (pad != null) {
							//    pad.SetFrame(z);
							//  }
							//}
		...
		private void addFrameButton_Click(object sender, EventArgs e) {
			_changed = true;
			for (int i = 0; i < launchPads.Count; i++) {
				launchPads[i].AddFrame(decimal.Parse(textBox1.Text), panel1.FormControls[i] as LightPad);
			}
			UpdateFrames();
		}
		private void UpdateFrames() {
			...
			foreach (FrameData frame in launchPads[0].FrameData) {
				frameListBox.ControlItems.Add(frame);
			}
		...
		private void checkedListBox1_Click(object sender, EventArgs e) {
			_changed = true;
			for (int i = 0; i < panel1.FormControls.Count; i++) {
				(panel1.FormControls[i] as LightPad).SetFrame(frameListBox.SelectedIndex);
		...
		private PopupResult Save() {
			...
			//  if (dialog.ShowDialog() == PopupResult.Ok) {
			//    SaveFileDialogView(dialog.FileName, launchPads);
			...
			//} else {
			//  _changed = false;
			//  SaveFileDialogView(filePath, launchPads);
			//  return PopupResult.Ok;
			//}
		...
		private void OverWritePadFrame(List<Launchpad> launchPads, FormControl panel1) {
			for (int i = 0; i < panel1.FormControls.Count; i++) {
				(panel1.FormControls[i] as LightPad).LightData.FrameData = launchPads[i].FrameData;
			}
		}
		...
		private void openDisplayFileButton_Click(object sender, EventArgs e) {
			...
			if (launchPads[0].FrameData.Count > 0 && _changed) {
				...
				while (!saveDeniedOrDone) {
					switch (Alert.SendAlert("You have unsaved changes. Would you like to save them?", "Song Illustrator", TypeOfAlert.Information, AlertButtons.YesNoCancel)) {
			...
			//  dialog.Filter = "Song Illustrator Files (*.slif)|*.slif";
			//  if (dialog.ShowDialog() == PopupResult.OK) {
			//    OpenFileDialogView(dialog.FileName);
			//    filePath = dialog.FileName;
			//  }
			}
		}
		...
		//    shiftDown = true;
		//    foreach (MacroButton button in panel1.FormControls) {
		...
		private void new_Click(object sender, EventArgs e) {
			...
			if (continueWithNewFile) {
				for (int i = 0; i < launchPads.Count; i++) {
					...
					(panel1.FormControls[i] as LightPad).LightData.FrameData.Clear();
				}
				frameListBox.ControlItems.Clear();
		...
		private void deleteButton_Click(object sender, EventArgs e) {
			_changed = true;
			if (Alert.SendAlert("Are you sure you want to delete these frames? (" + frameListBox.CheckedItems.Count + ")", "Delete Item?", TypeOfAlert.Information, AlertButtons.YesNo) == PopupResult.Ok) {
				for (int i = frameListBox.CheckedItems.Count - 1; i >= 0; i--) {
					...
					frameListBox.ControlItems.Remove(frameListBox.CheckedItems[i]);
					foreach (LightPad lightPad in panel1.FormControls) {
		...
		private void createListButton_Click(object sender, EventArgs e) {
			...
			bool loop = true;
			//OpenFileDialogView openDialog = new OpenFileDialogView();
			...
			//  }
			//  switch (Alert.SendAlert("Would you like to add more display files?", "Create Playlist", TypeOfAlert.Question, AlertButtons.YesNo)) {
			//    case PopupResult.No:
			//      loop = false;
			//      break;
			//  }
			//}
			//SaveFileDialogView saveDialog = new SaveFileDialogView();
		...
		private void Form1_Load(object sender, EventArgs e) {
			...
			_lowestDivisor = 1;
			GeneratePads(launchPads, panel1);
			mediaPlayer = MediaPlayerLoader.LoadMediaPlayer("");
			//(mediaPlayer as FormControl).Dock = System.Windows.Forms.DockStyle.Bottom;
			//(mediaPlayer as FormControl).ControlLocation = new ControlLocation(0, 607);
			//(mediaPlayer as FormControl).Name = "panel2";
			//(mediaPlayer as FormControl).ControlSize = new ControlSize(684, 55);
			//(mediaPlayer as FormControl).TabIndex = 92;
			...
			FormControls.Add(mediaPlayer as FormControl);
		}
		...
		public void GeneratePads(List<Launchpad> lightDataList, FormControl FormControl) {
			if (lightDataList.Count > _lowestDivisor * _lowestDivisor) {
				_lowestDivisor++;
			}
			int widthHeight = FormControl.Height / _lowestDivisor;
			...
			for (int i = 0; i < lightDataList.Count; i++) {
				...
				if (i > panel1.FormControls.Count) {
					(panel1.FormControls[i] as LightPad).Port = lightDataList[i].Port;
				}
				...
				lightPad.ControlLocation = new ControlLocation(lightPad.ControlSize.Height * widthProgression++, lightPad.ControlSize.Height * heightProgression);
				FormControl.FormControls.Add(lightPad);
		...
		private void displayButton2_Click(object sender, EventArgs e) {
			//Sync ButtonView.
			_changed = true;
			if (Alert.SendAlert("Are you sure you want to syncronize frames? It will re-increment the selected frames based on a delay of " + textBox2.Text + ".", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.YesNo) == PopupResult.Yes) {
				int frameIndex = 0;
				List<FrameData> frameTimes = new List<FrameData>();
				foreach (FrameData item in frameListBox.CheckedItems) {
					frameTimes.Add(item);
		...
		public void OverlayButtons(Stream bitmapStream, List<List<MacroButton>> FormControls) {
			...
			//  OverlayButtons(imageStream, FormControls[i]);
			//}
		}
		...
		private void shiftButton_Click(object sender, EventArgs e) {
			...
			if (frameListBox.CheckedItems.Count > 0) {
				if (Alert.SendAlert("Are you sure you want to shift the frames? It will place the selected frames starting from " + textBox1.Text + "ms.", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.YesNo) == PopupResult.Yes) {
					if (!string.IsNullOrEmpty(textBox1.Text)) {
						try {
							...
							Alert.SendAlert("Invalid number!", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
						}
					} else {
						Alert.SendAlert("Can't shift frames by an empty value!", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
					}
				}
			} else {
				Alert.SendAlert("You haven't checked off any frames to shift!", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
			}
		}
		...
		private PopupResult ExportMidi(List<FrameData> midiFrames) {
			...
			return PopupResult.None;
#else
			Alert.SendAlert("Incomplete Feature", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
			return PopupResult.Cancel;
#endif
		}
		...
		public void addLaunchpadToolStripMenuItem_Click(object sender, EventArgs e) {
			if (launchPads[0].FrameData.Count < 1) {
				...
				Alert.SendAlert("Cannot add Launchpads once frames have been added.", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
			}
		}
		...
		private void removeLaunchpadToolStripMenuItem_Click(object sender, EventArgs e) {
			if (launchPads.Count > 1) {
				if (frameListBox.ControlItems.Count <= 0) {
					...
					Alert.SendAlert("Cannot remove Launchpads once frames have been added.", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
				}
			} else {
				Alert.SendAlert("Cannot remove any more Launchpads.", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
			}
		}
		...
		private void addAFrameToolStripMenuItem_Click(object sender, EventArgs e) {
			...
			//figurePainter.TutorialFigures.Add(new TutorialFigure(panel1.FormControls, "Create an image on the canvas here by double clicking the virtual buttons."));
		...
		private void editCurrentFrameToolStripMenuItem_Click(object sender, EventArgs e) {
			if (launchPads[0].FrameData.Count > 0) {
				...
				Alert.SendAlert("No frames to edit!", "Song Illustrator", TypeOfAlert.Warning, AlertButtons.Ok);
			}
		}
		...
		private List<FormControl> _formControls = new List<FormControl>();
